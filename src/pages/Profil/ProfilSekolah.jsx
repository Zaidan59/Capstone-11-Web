...
export default function ProfilSekolah() {
  ...
  const linkedSppgId = sppg?.sppgId ?? sppg?.id ?? null;
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        const resSekolah = await getSekolahById(idSekolah);
        if (cancelled) return;

        const sekolahData = normalizeSekolahData(resSekolah?.data?.data ?? resSekolah?.data ?? DEFAULT_SEKOLAH);
        setSekolah(sekolahData);
        setLoading(false);

        const [resSppg, resMenu, resDokumentasi, resNutrisi, resCatatan] = await Promise.allSettled([
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            <div className="bg-white rounded-2xl px-8 py-8 flex items-center gap-6">
              <ImageBox src={sekolah.foto} alt={sekolah.nama} className="w-36 h-24 rounded-xl object-cover flex-shrink-0" />
                    ...
                    {menuHarian.map((item, index) => (
                      <div key={index} className="bg-white border border-gray-100 rounded-xl p-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          {item.isToday && <span className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-medium">Today</span>}
                        </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {dokumentasi.slice(0, 3).map((doc, i) => (
                      <div key={i} className="relative rounded-xl overflow-hidden aspect-[4/3]">
                        <ImageBox src={doc.fotoUrl || doc.foto} alt={doc.caption} className="w-full h-full object-cover" />
                        <div className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-[9px] px-1.5 py-1 leading-tight">{getDisplayValue(doc.caption)}</div>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    {catatan.map((c, i) => (
                      <div key={i} className="border border-gray-100 bg-white rounded-xl p-3.5 flex items-start gap-3">
                        ...
                        <div>
                          ...
                          {c.kutipan && <p className="text-[12px] text-gray-500 mt-1.5">"{c.kutipan}"</p>}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {sppg && (
                  <div>
                    ...
                    <div className="bg-blue-50 rounded-xl p-4">
                      ...
                      <button
                        type="button"
                        onClick={() => linkedSppgId && navigate(`/profil/sppg/${linkedSppgId}`)}
                        disabled={!linkedSppgId}
                        className="w-full bg-blue-600 hover:bg-blue-700 active:scale-95 transition-all text-white text-sm font-semibold py-2.5 rounded-xl flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                      >
                        Lihat Profil SPPG
