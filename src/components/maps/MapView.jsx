...
import { getLatLng } from "../../utils/mapUtils";
...
function MapController({ selectedItem }) {
  ...
  useEffect(() => {
    ...
    map.flyTo(getLatLng(selectedItem), 14, { duration: 0.65 });
  }, [map, selectedItem]);
...
function MapProjection({ selectedItem, onClear, onPointChange }) {
  ...
  const updatePoint = useCallback(() => {
    ...
    const point = map.latLngToContainerPoint(getLatLng(selectedItem));
...
export default function MapView({ kitchens, schools, links, loading, error }) {
  const [filter, setFilter] = useState("all");
  const [selected, setSelected] = useState(null);
  const [popupPoint, setPopupPoint] = useState(null);

  const allItems = useMemo(() => [...kitchens, ...schools], [kitchens, schools]);
  ...
  const visibleItems = useMemo(() => {
    if (filter === "sppg") return kitchens;
    if (filter === "school") return schools;
    return allItems;
  }, [allItems, filter, kitchens, schools]);

  const routeLines = useMemo(() => links.map((link) => [link.from, link.to]), [links]);
  ...
  return (
    <main className="relative h-[calc(100vh-3.5rem)] w-full overflow-hidden bg-slate-950">
      ...
      {loading ? (
        <div className="absolute right-5 top-5 z-[500] rounded-lg bg-white/90 px-3 py-2 text-xs font-bold text-slate-700 shadow">
          Memuat data peta...
        </div>
      ) : null}

      {error ? (
        <div className="absolute right-5 top-5 z-[500] rounded-lg bg-red-50 px-3 py-2 text-xs font-bold text-red-700 shadow">
